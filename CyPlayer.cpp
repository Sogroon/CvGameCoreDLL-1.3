...
/* Afforess	                     END                                                            */
/************************************************************************************************/
	return m_pPlayer ? new CyUnit(m_pPlayer->initUnit((UnitTypes) iIndex, iX, iY, eUnitAI, eFacingDirection, GC.getGameINLINE().getSorenRandNum(10000, "AI Unit Birthmark 30"))) : NULL;
}
...
std::wstring CyPlayer::getNewCivName()
{
	return m_pPlayer ? m_pPlayer->getNewCivName() : std::wstring();
}
...
int CyPlayer::getCurrentInflationCostModifier(void)
{
	return m_pPlayer ? m_pPlayer->getCurrentInflationCostModifier() : 0;
}

int CyPlayer::getEquilibriumInflationCostModifier(void)
{
...
int CyPlayer::getInterest() 
{
	return m_pPlayer ? m_pPlayer->getInterest() : -1;
}
...
int CyPlayer::getStateReligionHealth()
{
	return m_pPlayer ? m_pPlayer->getStateReligionHealth() : -1;
}
...
int CyPlayer::calculateTributeReceived()
{
	return (m_pPlayer ? m_pPlayer->calculateTributeReceived() : 0);
}

int CyPlayer::getTributeReceived(int /*PlayerTypes*/ ePlayer)
{
	return (m_pPlayer ? m_pPlayer->calculateTributeReceived((PlayerTypes)ePlayer) : 0);
}

int CyPlayer::calculateTributePayment()
{
	return (m_pPlayer ? m_pPlayer->calculateTributePayment() : 0);
}

int CyPlayer::calculateTotalForeignConnectednessSeized()
{
	return (m_pPlayer ? m_pPlayer->calculateTotalForeignConnectednessSeized() : 0);
}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
DiplomacyRequestAction CyPlayer::getRequestAction(int ePlayer, DiplomacyRequestType eRequestType)
{
	return m_pPlayer ? m_pPlayer->getRequestAction((PlayerTypes)ePlayer, eRequestType) : DEFAULT_ACTION;
}

void CyPlayer::setRequestAction(int ePlayer, DiplomacyRequestType eRequestType, int eAction)
{
	if (m_pPlayer)
		m_pPlayer->setRequestAction((PlayerTypes)ePlayer, eRequestType, (DiplomacyRequestAction)eAction);
}

bool CyPlayer::isAllowsAmbassadors()
{
	return m_pPlayer ? m_pPlayer->isAllowsAmbassadors() : false;
}
