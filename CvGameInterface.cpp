...
void CvGame::cycleCities(bool bForward, bool bAdd) const
{
	...
	{
		//Afforess - MP Resync
		//Changes for new ID system for cities
		/*
		...
		*/
		...
		//Can not rely on iLoop iteration ids to nessecarily be in order
		bool bFound = false;
		for (pLoopCity = GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).firstCity(&iLoop, !bForward); pLoopCity != NULL; pLoopCity = GET_PLAYER(pHeadSelectedCity->getOwnerINLINE()).nextCity(&iLoop, !bForward))
		{
			if (bFound)
			{
				break;
			}
			else if (pLoopCity == pHeadSelectedCity)
			{
				bFound = true;
			}
		}
