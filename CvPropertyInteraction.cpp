...
void CvPropertyInteractionInhibitedGrowth::read(FDataStreamBase *pStream)
{
	CvPropertyInteraction::read(pStream);
	pStream->Read(&m_iGrowthPercent);
	pStream->Read(&m_iInhibitionPercent);
}

void CvPropertyInteractionInhibitedGrowth::write(FDataStreamBase *pStream)
{
	CvPropertyInteraction::write(pStream);
	pStream->Write(m_iGrowthPercent);
	pStream->Write(m_iInhibitionPercent);
}
...
bool CvPropertyInteractionInhibitedGrowth::read(CvXMLLoadUtility *pXML)
{
	...
	pXML->GetChildXmlValByName(&m_iInhibitionPercent, "iInhibitionPercent");
	return true;
}
...
void CvPropertyInteractionConvertPercent::read(FDataStreamBase *pStream)
{
	CvPropertyInteraction::read(pStream);
	pStream->Read(&m_iPercent);
	pStream->Read(&m_iNoConvertAmount);
}

void CvPropertyInteractionConvertPercent::write(FDataStreamBase *pStream)
{
	CvPropertyInteraction::write(pStream);
	pStream->Write(m_iPercent);
	pStream->Write(m_iNoConvertAmount);
}
...
bool CvPropertyInteractionConvertPercent::read(CvXMLLoadUtility *pXML)
{
	...
	pXML->GetChildXmlValByName(&m_iNoConvertAmount, "iNoConvertAmount");
	return true;
}
