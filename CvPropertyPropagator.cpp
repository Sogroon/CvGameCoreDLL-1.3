...
void CvPropertyPropagator::read(FDataStreamBase *pStream)
{
	...
	if (bExpr)
	{
		m_pExprTargetCondition = BoolExpr::readExpression(pStream);
	}
}
...
void CvPropertyPropagator::write(FDataStreamBase *pStream)
{
	...
	if (m_pExprTargetCondition)
	{
		pStream->Write(true);
		m_pExprTargetCondition->write(pStream);
	}
	else
	{
		pStream->Write(false);
...
//	m_eProperty = (PropertyTypes) pXML->FindInInfoClass(szTextVal);
	...
	if (GETXML->SetToChildByTagName(pXML->GetXML(),"TargetCondition"))
	{
