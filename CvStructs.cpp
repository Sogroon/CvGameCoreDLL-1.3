...
//Afforess - MP Resync
void EventTriggeredData::resync(bool bWrite, ByteBuffer* pBuffer)
{
	RESYNC_INT(bWrite, pBuffer, m_iId);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eTrigger, EventTriggerTypes);
	RESYNC_INT(bWrite, pBuffer, m_iTurn);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_ePlayer, PlayerTypes);
	RESYNC_INT(bWrite, pBuffer, m_iCityId);
	RESYNC_INT(bWrite, pBuffer, m_iPlotX);
	RESYNC_INT(bWrite, pBuffer, m_iPlotY);
	RESYNC_INT(bWrite, pBuffer, m_iUnitId);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eOtherPlayer, PlayerTypes);
	RESYNC_INT(bWrite, pBuffer, m_iOtherPlayerCityId);
	RESYNC_INT(bWrite, pBuffer, m_bExpired);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eReligion, ReligionTypes);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eCorporation, CorporationTypes);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eBuilding, BuildingTypes);
	RESYNC_STRING(bWrite, pBuffer, m_szText);
	RESYNC_STRING(bWrite, pBuffer, m_szGlobalText);
}
...
void VoteSelectionData::resync(bool bWrite, ByteBuffer* pBuffer)
{
	RESYNC_INT(bWrite, pBuffer, iId);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, eVoteSource, VoteSourceTypes);
	if (bWrite)
	{
		pBuffer->putInt(aVoteOptions.size());
		for (std::vector<VoteSelectionSubData>::iterator it = aVoteOptions.begin(); it != aVoteOptions.end(); ++it)
		{
			pBuffer->putInt((*it).eVote);
			pBuffer->putInt((*it).ePlayer);
			pBuffer->putInt((*it).iCityId);
			pBuffer->putInt((*it).eOtherPlayer);
			pBuffer->putString((*it).szText);
		}
	}
	else
	{
		...
		for (int i = 0; i < iSize; i++)
		{
			VoteSelectionSubData data;
			data.eVote = (VoteTypes)pBuffer->getInt();
			data.ePlayer = (PlayerTypes)pBuffer->getInt();
			data.iCityId = pBuffer->getInt();
			data.eOtherPlayer = (PlayerTypes)pBuffer->getInt();
			pBuffer->getString(data.szText);
		}
	}
}
...
void VoteTriggeredData::resync(bool bWrite, ByteBuffer* pBuffer)
{
	RESYNC_INT(bWrite, pBuffer, iId);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, eVoteSource, VoteSourceTypes);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, kVoteOption.eVote, VoteTypes);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, kVoteOption.ePlayer, PlayerTypes);
	RESYNC_INT(bWrite, pBuffer, kVoteOption.iCityId);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, kVoteOption.eOtherPlayer, PlayerTypes);
	RESYNC_STRING(bWrite, pBuffer, kVoteOption.szText);
}
