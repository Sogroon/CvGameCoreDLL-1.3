...
// Pseudo-unicode: Initialization of Unicode->Locale tables --Header-- (Nightinggale, dbkblk; 2015-01)
...
bool CvXMLLoadUtility::ReadGlobalDefines(const TCHAR* szXMLFileName, CvCacheObject* cache)
{
	...
	if (!gDLL->cacheRead(cache, szXMLFileName))			// src data file name
	{
		...
		if (bLoaded)
		{
			...
			if (GETXML->LocateNode(m_pFXml,"Civ4Defines/Define"))
			{
				int i;	// loop counter
				// get the number of other Define tags in the xml file
				int iNumDefines = GETXML->GetNumSiblings(m_pFXml);
						...
						if (GetChildXmlVal(szName))
						{
							...
							if (GETXML->NextSibling(GetXML()))
							{
								...
								if (SkipToNextVal())
								{
									...
									if (GETXML->GetLastLocatedNodeTagName(GetXML(),szNodeType))
									{
...
#else
									// if we successfuly get the node type for the current tag
									if (GETXML->GetLastLocatedNodeType(GetXML(),szNodeType))
									{
										// if the node type of the current tag isn't null
										if (strcmp(szNodeType,"")!=0)
										{
...
/************************************************************************************************/
	...
	// Check if we are allowed to use XML Caching
	if (GC.getDefineBOOL("DO_CACHING"))
	{
		GC.setDoCaching(true);
		CvXMLLoadUtilitySetMod* pSetMod = new CvXMLLoadUtilitySetMod;
		pSetMod->checkCacheState();
		SAFE_DELETE(pSetMod);
	}
	else
	{
		GC.setDoCaching(false);
...
bool CvXMLLoadUtility::SetGlobalArtDefines()
{
	...
	LoadGlobalClassInfo(ARTFILEMGR.getMovieArtInfo(), "CIV4ArtDefines_Movie", "Art", "Civ4ArtDefines/MovieArtInfos/MovieArtInfo", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
	LoadGlobalClassInfo(ARTFILEMGR.getMiscArtInfo(), "CIV4ArtDefines_Misc", "Art", "Civ4ArtDefines/MiscArtInfos/MiscArtInfo", false);
	LoadGlobalClassInfo(ARTFILEMGR.getUnitArtInfo(), "CIV4ArtDefines_Unit", "Art", "Civ4ArtDefines/UnitArtInfos/UnitArtInfo", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
	LoadGlobalClassInfo(ARTFILEMGR.getBuildingArtInfo(), "CIV4ArtDefines_Building", "Art", "Civ4ArtDefines/BuildingArtInfos/BuildingArtInfo", false, &CvDLLUtilityIFaceBase::createBuildingInfoCacheObject);
	LoadGlobalClassInfo(ARTFILEMGR.getCivilizationArtInfo(), "CIV4ArtDefines_Civilization", "Art", "Civ4ArtDefines/CivilizationArtInfos/CivilizationArtInfo", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
	LoadGlobalClassInfo(ARTFILEMGR.getLeaderheadArtInfo(), "CIV4ArtDefines_Leaderhead", "Art", "Civ4ArtDefines/LeaderheadArtInfos/LeaderheadArtInfo", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
	LoadGlobalClassInfo(ARTFILEMGR.getBonusArtInfo(), "CIV4ArtDefines_Bonus", "Art", "Civ4ArtDefines/BonusArtInfos/BonusArtInfo", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
...
/************************************************************************************************/
		...
		if (szLanguage == "Default")
		{
			switch (GAMETEXT.getCurrentLanguage())
			{
				...
				case 18:
					szLanguage = "Brazilian";
					break;

				case 19:
					szLanguage = "ChineseTrad";
					break;
				...
				case 22:
					szLanguage = "Bulgarian";
					break;

				case 23:
					szLanguage = "Ukrainian";
					break;
...
/***** Parallel Maps - Begin *****/
/*********************************/
	LoadGlobalClassInfo(GC.getMapInfo(), "CIV4MapInfo", "GameInfo", "Civ4MapInfos/MapInfos/MapInfo", true);
	LoadGlobalClassInfo(GC.getMapSwitchInfo(), "CIV4MapSwitchInfo", "GameInfo", "Civ4MapSwitchInfos/MapSwitchInfos/MapSwitchInfo", false);
...
/************************************************************************************************/
	...
	LoadGlobalClassInfo(GC.getColorInfo(), "CIV4ColorVals", "Interface", "Civ4ColorVals/ColorVals/ColorVal", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
	...
	LoadGlobalClassInfo(GC.getUnitClassInfo(), "CIV4UnitClassInfos", "Units", "Civ4UnitClassInfos/UnitClassInfos/UnitClassInfo", false, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject);
	...
	LoadGlobalClassInfo(GC.getTechInfo(), "CIV4TechInfos", "Technologies", "Civ4TechInfos/TechInfos/TechInfo", true, &CvDLLUtilityIFaceBase::createTechInfoCacheObject, GC.getTechInfoReplacements());
...
/************************************************************************************************/
	...
	LoadGlobalClassInfo(GC.getBuildingClassInfo(), "CIV4BuildingClassInfos", "Buildings", "Civ4BuildingClassInfos/BuildingClassInfos/BuildingClassInfo", false, &CvDLLUtilityIFaceBase::createBuildingInfoCacheObject);
	...
	if (!GC.getDoCaching() || GC.getWriteCache())
	{
...
/* SORT_ALPHABET                           END                                                  */
/************************************************************************************************/
	LoadGlobalClassInfo(GC.getPlayerColorInfo(), "CIV4PlayerColorInfos", "Interface", "Civ4PlayerColorInfos/PlayerColorInfos/PlayerColorInfo", false, &CvDLLUtilityIFaceBase::createLeaderHeadInfoCacheObject);
...
/************************************************************************************************/
	...
	LoadGlobalClassInfo(GC.getUnitInfo(), "CIV4UnitInfos", "Units", "Civ4UnitInfos/UnitInfos/UnitInfo", true, &CvDLLUtilityIFaceBase::createUnitInfoCacheObject, GC.getUnitInfoReplacements());
...
/************************************************************************************************/
	...
	LoadGlobalClassInfo(GC.getCivNameInfo(), "CIV4CivNameInfos", "Civilizations", "Civ4CivNameInfos/CivNameInfos/CivNameInfo", false);
/************************************************************************************************/
/* SORT_ALPHABET                           11/17/07                                MRGENIE      */
/*                                                                                              */
/*                                                                                              */
...
/************************************************************************************************/
	...
	//LoadGlobalClassInfo(GC.getTraitOptionEditsInfo(), "CIV4TraitOptionEditsInfos", "Civilizations", "Civ4TraitOptionEditsInfos/TraitOptionEditsInfos/TraitOptionEditsInfo", true);
...
void CvXMLLoadUtility::SetGlobalStringArray(CvString **ppszString, char* szTagName, int* iNumVals, bool bUseEnum)
{
	...
	if (GETXML->LocateNode(m_pFXml,szTagName))
	{
		if (!bUseEnum)
		{
			...
			*iNumVals = GETXML->NumOfElementsByTagName(m_pFXml,szTagName);
		}
...
void CvXMLLoadUtility::SetGameText(const char* szTextGroup, const char* szTagName)
{
	...
	if (GETXML->LocateNode(m_pFXml, szTextGroup)) // Get the Text Group 1st
	{
		int iNumVals = GETXML->GetNumChildren(m_pFXml);	// Get the number of Children that the Text Group has
		GETXML->LocateNode(m_pFXml, szTagName); // Now switch to the TEXT Tag
		GETXML->SetToParent(m_pFXml);
		GETXML->SetToChild(m_pFXml);

		// loop through each tag
		/* Pseudo-unicode (Nigtinggale) --Start--*/
		for (i = 0; i < iNumVals; i++)
		{
			CvGameText textInfo;
			...
			{
				logMsg("Reading %s", GC.getCurrentXMLFile().GetCString());
				gDLL->addText(textInfo.getType() /*id*/, textInfo.getText(), textInfo.getGender(), textInfo.getPlural());
				if (!GETXML->NextSibling(m_pFXml) && i != iNumVals - 1)
				{
					char	szMessage[1024];
		...
		/* Pseudo-unicode (Nigtinggale) --End--*/
		/*for (i=0; i < iNumVals; i++)
		{
			CvGameText textInfo;
			textInfo.read(this);
...
//------------------------------------------------------------------------------------------------------
template <class T>
void CvXMLLoadUtility::SetGlobalClassInfo(std::vector<T*>& aInfos, const char* szTagName, bool bTwoPass, CvInfoReplacements<T>* pReplacements)
{
	char szLog[256];
...
/************************************************************************************************/
template <class T>
void CvXMLLoadUtility::SetGlobalClassInfoTwoPassReplacement(std::vector<T*>& aInfos, const char* szTagName, CvInfoReplacements<T>* pReplacements)
{
	char szLog[256];
	sprintf(szLog, "SetGlobalClassInfo (%s)", szTagName);
	PROFILE(szLog);
...
template <class T>
void CvXMLLoadUtility::LoadGlobalClassInfo(std::vector<T*>& aInfos, const char* szFileRoot, const char* szFileDirectory, const char* szXmlPath, bool bTwoPass, CvCacheObject* (CvDLLUtilityIFaceBase::*pArgFunction) (const TCHAR*), CvInfoReplacements<T>* pReplacements)
{
	bool bLoaded = false;
...
/************************************************************************************************/
	...
	{
		// AIAndy: Replaced with manual caching
		...
		if (GC.getDoCaching() && !GC.getWriteCache())
		{
			...
			if (cacheStream.IsOpen())
			{
				...
				{
					T* pClassInfo = new T();
					CvInfoBase* pInfo = (CvInfoBase*)pClassInfo; // The compiler does not look into the base class by itself so the info classes without stream read cause a compile error otherwise
					pInfo->read(&cacheStream);
					aInfos.push_back(pClassInfo);
					if (NULL != pClassInfo->getType())
					{
				...
				logMsg("Read %s from cache", szFileRoot);
				bLoaded = true;

				// Load the associated replacement cache if present for this info class
				if (pReplacements)
				{
					szCacheFile = szModDirectory + "Cache\\" + szFileRoot + "Repl.dat";
					cacheStream.OpenRead(szCacheFile);
					if (cacheStream.IsOpen())
					{
						pReplacements->read(&cacheStream);
						cacheStream.Close();
					}
				}
			}
			...
			{
				//	We didn't manage to load from cache so the cache file must be missing which implies the cache
				//	as a whole is corrupted, so write a new one and stop trying to load from cache
...
int CvXMLLoadUtility::SetYields(int** ppiYield)
{
	...
	int *piYield = NULL;	// local pointer for the yield memory
...
void CvXMLLoadUtility::SetFeatureStruct(int** ppiFeatureTech, int** ppiFeatureTime, int** ppiFeatureProduction, bool** ppbFeatureRemove, bool** ppbNoTechCanRemoveWithNoProductionGain)
{
	...
	if (GETXML->SetToChildByTagName(m_pFXml,"FeatureStructs"))
	{
		iNumSibs = GETXML->GetNumChildren(m_pFXml);

		if (0 < iNumSibs)
		{
			if (GETXML->SetToChildByTagName(m_pFXml,"FeatureStruct"))
			{
...
/************************************************************************************************/
							...
							if (GETXML->SetToChildByTagName(m_pFXml,"YieldChanges"))
							{
...
{
	// Pseudo-unicode: Initialization of Unicode->Locale tables --Start-- (Nightinggale, dbkblk; 2015-01)
	setConversionTableUnicode();
	// Pseudo-unicode: Initialization of Unicode->Locale tables --End-- (Nightinggale, dbkblk; 2015-01)
/************************************************************************************************/
...
/************************************************************************************************/
	...
	bool bLoaded = LoadCivXml(m_pFXml, CvString::format("%s\\MLF_%s.xml", szModDirectory.c_str(), szFileRoot));
	...
	{
		if ( GETXML->LocateNode(m_pFXml,"Civ4ModularLoadControls/DefaultConfiguration"))
		{
			// call the function that sets the FXml pointer to the first non-comment child of
			// the current tag and gets the value of that new node
#ifdef USE_RAPID_XML
...
bool CvXMLLoadUtility::SetModLoadControlInfo(std::vector<T*>& aInfos, const char* szTagName, CvString szConfigString, CvString szDirDepth, int iDirDepth)
{
	...
	if ( GETXML->LocateNode(m_pFXml,"Civ4ModularLoadControls/ConfigurationInfos/ConfigurationInfo"))
	{		
		// loop through each tag
		do
		{
...
void CvXMLLoadUtility::SetStringWithChildList(int* iNumEntries, std::vector<CvString>* aszXMLLoad)
{
    ...
    if (SkipToNextVal())
    {
        int iNumSibs = GETXML->GetNumChildren(m_pFXml);
        if (0 < iNumSibs)
        {
            if (GetChildXmlVal(szTextVal))
            {
