...
//Afforess - MP resync
void CvPlotGroup::resync(bool bWrite, ByteBuffer* pBuffer)
{
	...
	RESYNC_OPTIONAL_ARRAY(bWrite, pBuffer, GC.getNumBuildingInfos(), m_paiNumFreeTradeRegionBuildings, int);
	RESYNC_INT(bWrite, pBuffer, m_numPlots);
	RESYNC_INT(bWrite, pBuffer, m_seedPlotX);
	RESYNC_INT(bWrite, pBuffer, m_seedPlotY);
}
...
void CvPlotGroup::startBulkRecalculate(void)
{
	m_bulkRecalcStartSeq = m_recalcSeqForSession;
}

void CvPlotGroup::endBulkRecalculate(void)
{
