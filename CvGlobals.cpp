...
volatile int giProfilerDisabled = 0;  // set to 1 or more in threaded areas as the profiler is not thread safe
...
m_iCITY_FREE_CULTURE_GROWTH_FACTOR(22),
...
/***** Parallel Maps - Begin *****/
/*********************************/
	m_maps.push_back(new CvMap(INITIAL_MAP));
/*******************************/
/***** Parallel Maps - End *****/
/*******************************/
...
bool cvInternalGlobals::multiMapsEnabled(void) const
{
	return m_bMultimapsEnabled;
}

bool cvInternalGlobals::viewportsEnabled(void) const
{
	return m_bViewportsEnabled;
}

bool cvInternalGlobals::getReprocessGreatWallDynamically(void) const
{
...
std::vector<CvMapInfo*>& cvInternalGlobals::getMapInfo()
{
	return m_paMapInfo;
}

std::vector<CvMapSwitchInfo*>& cvInternalGlobals::getMapSwitchInfo()
{
...
bool cvInternalGlobals::getResourceLayer(void) const
{
...
bool cvInternalGlobals::getDoCaching()
{
	return m_bCache;
}

void cvInternalGlobals::setDoCaching(bool bCache)
{
	m_bCache = bCache;
}

bool cvInternalGlobals::getWriteCache()
{
	return m_bWriteCache;
}

void cvInternalGlobals::setWriteCache(bool bWriteCache)
{
	m_bWriteCache = bWriteCache;
}
...
{
	m_UnitInfoReplacements.updateReplacements(getUnitInfo());
	m_TechInfoReplacements.updateReplacements(getTechInfo());
}
...
std::vector<CvUnitCombatInfo*>& cvInternalGlobals::getUnitCombatInfo()
{
...
int cvInternalGlobals::getNumCivNameInfos()
{
	return (int)m_paCivNameInfo.size();
}

std::vector<CvCivNameInfo*>& cvInternalGlobals::getCivNameInfo()
{
	return m_paCivNameInfo;
}

CvCivNameInfo& cvInternalGlobals::getCivNameInfo(CivNameTypes e)
{
	FAssert(e > -1);
	FAssert(e < GC.getNumCivNameInfos());
	return *(m_paCivNameInfo[e]);
}
...
void cvInternalGlobals::registerUnitAIs(void)
{
	...
	REGISTER_UNITAI(UNITAI_DIPLOMAT);
}
...
//Afforess added defValue
float cvInternalGlobals::getDefineFLOAT( const char * szName, float defValue) const
{
	float fReturn = 0;
...
{
	const char * szReturn = NULL;
	if (GC.getDefinesVarSystem()->GetValue( szName, szReturn ))
	{
		return szReturn;
	}
...
int cvInternalGlobals::getRECON_VISIBILITY_RANGE() const
{
...
void cvInternalGlobals::clearSigns(void)
{
...
bool cvInternalGlobals::bugInitCalled(void) const
{
...
TeamTypes cvInternalGlobals::getTurnActiveTeam() const
{
	if (GC.getInitCore().getMPOption(MPOPTION_SIMULTANEOUS_TURNS))
	{
		return NO_TEAM;
	}
	return m_eActiveTeamTurn;
}

void cvInternalGlobals::setTurnActiveTeam(TeamTypes eTeam)
{
	m_eActiveTeamTurn = eTeam;
}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
int cvInternalGlobals::getCITY_FREE_CULTURE_GROWTH_FACTOR()
{
	return m_iCITY_FREE_CULTURE_GROWTH_FACTOR;
}

int cvInternalGlobals::getMAX_TRADE_ROUTES()
{
	return m_iMAX_TRADE_ROUTES;
}

bool cvInternalGlobals::isDISABLE_INFLATION()
{
	return m_bDISABLE_INFLATION;
}

bool cvInternalGlobals::isENABLE_RESYNC_CODE()
{
	return m_bENABLE_RESYNC_CODE;
}
/************************************************************************************************/
/* Mod Globals                        END                                           phungus420  */
/************************************************************************************************/
