...
class CvRandomSeedUpdate : public CvMessageData
{
public:
	CvRandomSeedUpdate(unsigned long ulMapSeed = 0, unsigned long ulSorenSeed = 0);
	virtual void Debug(char* szAddendum);
	virtual void Execute();
	virtual void PutInBuffer(FDataStreamBase* pStream);
	virtual void SetFromBuffer(FDataStreamBase* pStream);
private:
	unsigned long m_ulMapSeed;
	unsigned long m_ulSorenSeed;
};

class CvGameResyncPacket : public CvMessageData
{
public:
	CvGameResyncPacket(byte* pBuffer = NULL, int iBufferLen = -1, int iPacketNum = -1, int iNumPackets = -1, int OriginalBufferLen = -1);
	virtual ~CvGameResyncPacket();
	virtual void Debug(char* szAddendum);
	virtual void Execute();
	virtual void PutInBuffer(FDataStreamBase* pStream);
	virtual void SetFromBuffer(FDataStreamBase* pStream);
