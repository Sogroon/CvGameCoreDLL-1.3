...
bool CvEventReporter::kbdEvent(int evt, int key, int iCursorX, int iCursorY)
{
	...
	if ( !bResult )
	{
		...
		if ( gDLL->altKey() && gDLL->shiftKey() && evt == 7 && (key == 30 || key == 32) )
		{
			//Afforess: Switching the language back and forth causes the game to reload the game text, useful for updating civ text/strategies
			int iLanguage = gDLL->getCurrentLanguage();
			int iTempLanguage = (iLanguage > 0 ? iLanguage - 1 : iLanguage + 1);
			gDLL->setCurrentLanguage(iTempLanguage);
