...
void CvDeal::resync(bool bWrite, ByteBuffer* pBuffer)
{
	RESYNC_INT(bWrite, pBuffer, m_iID);
	RESYNC_INT(bWrite, pBuffer, m_iInitialGameTurn);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eFirstPlayer, PlayerTypes);
	RESYNC_INT_WITH_CAST(bWrite, pBuffer, m_eSecondPlayer, PlayerTypes);

	FDataStreamBuffer fBuffer;

	if (bWrite)
	{
		m_firstTrades.WriteNonWrapper(&fBuffer);
		m_secondTrades.WriteNonWrapper(&fBuffer);
		//Write size
	...
	else
	{
		...
		m_firstTrades.ReadNonWrapper(&fBuffer);
		m_secondTrades.ReadNonWrapper(&fBuffer);
	}
}
...
/************************************************************************************************/
		...
		if (GET_TEAM(GET_PLAYER(eToPlayer).getTeam()).isAtWar((TeamTypes)trade.m_iData))
		{
			// If our team is at war with the person we hired the war ally for, we aren't much of a war monger, we just wanted an ally
			GET_PLAYER(eToPlayer).changeWarMongererIndex(GC.getDefineINT("WARMONGER_INDEX_HIRED_WAR_ALLY", 200));
		}
		else
		{
			// If we incited  a war with a civ we aren't at war with, we are war mongerers
			GET_PLAYER(eToPlayer).changeWarMongererIndex(GC.getDefineINT("WARMONGER_INDEX_HIRED_WAR_ALLY", 1000));
		}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
...
/************************************************************************************************/
  case TRADE_EMBASSY:
    ...
    case TRADE_CONTACT:
		if (GC.getGameINLINE().isOption(GAMEOPTION_ADVANCED_DIPLOMACY))
		{
       		 GET_TEAM(GET_PLAYER(eToPlayer).getTeam()).meet((TeamTypes)trade.m_iData, true);
		}
        break;
	case TRADE_CEASE_RELATIONS:
		if (GC.getGameINLINE().isOption(GAMEOPTION_ADVANCED_DIPLOMACY))
		{
			int iTurns = GC.getGameINLINE().getCeaseRelationsTurnAmt();
			GET_PLAYER(eFromPlayer).setCeaseContactCounter((PlayerTypes)trade.m_iData, iTurns);
			GET_PLAYER((PlayerTypes)trade.m_iData).setCeaseContactCounter(eFromPlayer, iTurns);
			GET_PLAYER((PlayerTypes)trade.m_iData).AI_changeMemoryCount(eToPlayer, MEMORY_EVENT_BAD_TO_US, 2);
			GET_PLAYER((PlayerTypes)trade.m_iData).AI_changeMemoryCount(eFromPlayer, MEMORY_EVENT_BAD_TO_US, 2);

			GET_PLAYER(eFromPlayer).stopTradingWithTeam(GET_PLAYER((PlayerTypes)trade.m_iData).getTeam());

			GET_PLAYER(eFromPlayer).AI_changeMemoryCount((PlayerTypes)trade.m_iData, MEMORY_RECALLED_AMBASSADOR, -1);

			for (int i = 0; i < MAX_PLAYERS; i++)
			{
				if (GET_TEAM(GET_PLAYER((PlayerTypes)i).getTeam()).isHasMet(GET_PLAYER(eFromPlayer).getTeam()) && GET_TEAM(GET_PLAYER((PlayerTypes)i).getTeam()).isHasMet(GET_PLAYER((PlayerTypes)trade.m_iData).getTeam()))
				{
					MEMORY_TRACK_EXEMPT();

					szBuffer = gDLL->getText("TXT_KEY_TRADE_CEASE_RELATIONS_GLOBAL_MSG", GET_PLAYER(eToPlayer).getCivilizationDescriptionKey(), GET_PLAYER(eFromPlayer).getCivilizationDescriptionKey(), GET_PLAYER((PlayerTypes)trade.m_iData).getCivilizationDescriptionKey()).GetCString();
					AddDLLMessage(((PlayerTypes)i), true, GC.getEVENT_MESSAGE_TIME(), szBuffer, "AS2D_BUILD_BARRACKS", MESSAGE_TYPE_MAJOR_EVENT, NULL, (ColorTypes)GC.getInfoTypeForString("COLOR_YELLOW"));
				}
			}

			if (GET_TEAM(GET_PLAYER(eToPlayer).getTeam()).isAtWar(GET_PLAYER((PlayerTypes)trade.m_iData).getTeam()))
			{
				GET_PLAYER(eToPlayer).changeWarMongererIndex(GC.getDefineINT("WARMONGER_INDEX_HIRED_WAR_ALLY", 100) / 2);
			}
			else
			{
				GET_PLAYER(eToPlayer).changeWarMongererIndex(GC.getDefineINT("WARMONGER_INDEX_BRIBED_WAR_ALLY", 1000) / 5);
			}
		}
		break;
