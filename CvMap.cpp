...
void CvMap::read(FDataStreamBase* pStream)
{
	...
	//Afforess: read zobrist cache
	CvPlot::readZobristCache(&wrapper);
...
void CvMap::write(FDataStreamBase* pStream)
{
	...
	//Afforess: write zobrist cache
	CvPlot::writeZobristCache(&wrapper);
...
void CvMap::resync(bool bWrite, ByteBuffer* pBuffer)
{
	RESYNC_INT(bWrite, pBuffer, m_iGridWidth);
	RESYNC_INT(bWrite, pBuffer, m_iGridHeight);
	...
	RESYNC_INT(bWrite, pBuffer, m_iLandPlots);
	RESYNC_INT(bWrite, pBuffer, m_iOwnedPlots);
	RESYNC_INT(bWrite, pBuffer, m_iTopLatitude);
	RESYNC_INT(bWrite, pBuffer, m_iBottomLatitude);
	RESYNC_INT(bWrite, pBuffer, m_iNextRiverID);

	RESYNC_BOOL(bWrite, pBuffer, m_bWrapX);
	RESYNC_BOOL(bWrite, pBuffer, m_bWrapY);

	RESYNC_INT_ARRAY(bWrite, pBuffer, GC.getNumBonusInfos(), m_paiNumBonus);
	RESYNC_INT_ARRAY(bWrite, pBuffer, GC.getNumBonusInfos(), m_paiNumBonusOnLand);

	int iI;
	for (iI = 0; iI < numPlotsINLINE(); iI++)
	{
		m_pMapPlots[iI].resync(bWrite, pBuffer);
	}

	// call the read of the free list CvArea class allocations
	ResyncStreamableFFreeListTrashArray(bWrite, pBuffer, m_areas);
}
