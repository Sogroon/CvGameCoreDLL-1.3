...
bool CvXMLLoadUtility::GetXmlVal(char* pszVal, char* pszDefault)
{
	...
	if (SkipToNextVal())
	{
		...
		GETXML->GetLastNodeValue(m_pFXml,pszVal);
		return true;
	}
...
bool CvXMLLoadUtility::GetXmlVal(int* piVal, int iDefault)
{
	...
	*piVal = iDefault;
	...
	if (SkipToNextVal())
	{
		// get the type of the current xml node
		TCHAR s[32];
#ifndef USE_RAPID_XML
		GETXML->GetLastLocatedNodeType(m_pFXml, s);
		if (strcmp(s,""))
		{
			GETXML->GetLastNodeValue(m_pFXml, piVal);
		}
		else
#endif
		{
			// read as string, check if it is an int
			GETXML->GetLastNodeValue(m_pFXml, s);
			std::stringstream ss(s);
			if( (ss >> *piVal).fail() )
			{
				// not the right type, assume it is a global define
				*piVal = GC.getDefineINT(s);
			}
		}
		return true;
	}
...
bool CvXMLLoadUtility::GetXmlVal(float* pfVal, float fDefault)
{
	...
	*pfVal = fDefault;
	...
	if (SkipToNextVal())
	{
		...
		if (strcmp(s,""))
		{
			GETXML->GetLastNodeValue(m_pFXml, pfVal);
		}
		else
#endif
		{
			// read as string, check if it is a float
			GETXML->GetLastNodeValue(m_pFXml, s);
			std::stringstream ss(s);
			if( (ss >> *pfVal).fail() )
			{
				// not the right type, assume it is a global define
				*pfVal = GC.getDefineFLOAT(s);
			}
		}
		return true;
	}
...
bool CvXMLLoadUtility::GetXmlVal(bool* pbVal, bool bDefault)
{
	...
	*pbVal = bDefault;
	...
	if (SkipToNextVal())
	{
		...
		if (strcmp(s,""))
		{
			GETXML->GetLastNodeValue(m_pFXml, pbVal);
		}
		else
#endif
		{
...
bool CvXMLLoadUtility::GetChildXmlValByName(wchar* pszVal, const TCHAR* szName, wchar* pszDefault)
{
	...
	if (GETXML->SetToChildByTagName(m_pFXml,szName))
	{
			// get the string value of the current xml node
			GETXML->GetLastNodeValue(m_pFXml,pszVal);
