...
{
	// Removed. Cause a python bug at initialization. No need to set viewports at startup (dbkblk, 2015-02)
	if (m_pMap->getGridWidthINLINE() > 0 && !GC.bugInitCalled())
	{ // Disable viewport call if BUG isn't yet loaded
		PYTHON_ACCESS_LOCK_SCOPE

		//	Force-load the main interface BUG module so we can get at the viewport BUG settings
		CyArgsList argsList;
		argsList.add(wchar("BUG Main Interface"));
...
bool	CvViewport::isMidSwitch(void) const
{
