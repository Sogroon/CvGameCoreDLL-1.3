...
public:
	CvWString() {}
	CvWString(int iLen) { reserve(iLen); } 	// Asian executable compatibility (dbkblk, 2015-02)
...
//	CvWString(const __wchar_t* s) { if (s) *this = s; }
	...
	{ // Asian executable compatibility (dbkblk, 2015-02)
		if (s.c_str())
		{
	...
	int Replace(const CvWString& searchString, const CvWString& replaceString);
...
inline int CvWString::Replace(const CvWString& searchString, const CvWString& replaceString)
{
	int iCnt = 0;
	std::wstring::size_type pos = this->find(searchString, 0);
	int intLengthSearch = searchString.length();
