...
bool CyGame::getAscensionGateBuilt(int playaID)
{
	return m_pGame ? m_pGame->getAscensionGateBuilt(playaID) : false;
}
...
bool CyGame::isDarkAge() const
{
	return (NULL != m_pGame ? m_pGame->isDarkAge() : false);
}

bool CyGame::isDarkAgePending() const
{
	return (NULL != m_pGame ? m_pGame->isDarkAgePending() : false);
}
void CyGame::triggerDarkAge(float iDelayTurns, float iLenTurns, std::wstring szStartTxtKey, std::wstring szEndTxtKey)
{
	if (m_pGame)
		m_pGame->triggerDarkAge((int)iDelayTurns, (int)iLenTurns, CvWString(szStartTxtKey), CvWString(szEndTxtKey));
}

int CyGame::getNumDarkAgesEverOccurred() const
{
	return (NULL != m_pGame ? m_pGame->getNumDarkAgesEverOccurred() : 0);
}
/************************************************************************************************/
/* Afforess	                     END                                                            */
/************************************************************************************************/
